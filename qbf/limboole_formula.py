...
class Limboole_f_ctor:
    ...
    def get_qbf_formula(self, aut):
        ...
        if self.exist_vars:
            formula = SATformula("?", subf_list=[formula], quantifier = True, quantified_vars=self.exist_vars)
            #formula = Exists(self.exist_vars, formula)
        formula = SATformula("#",subf_list=[formula], quantifier = True, quantified_vars=self.univ_vars )
...
# NEW CONDITION PART
    ...
    def equivalence_f(self, left_f, right_f):
        a = SATformula("->", [left_f, right_f])
        b = SATformula("->", [right_f, left_f])
        return SATformula("&", [a, b])
    ...
    def laso_cycles(self, aut):
        ...
        for alist in self.scc_edg:
            for e in alist:
                ...
                src = str(e.src)
                dst = str(e.dst)
                c = SATformula("&", [SATformula("w_" + src), SATformula("w_" + dst)])
                impl = SATformula("->", [SATformula("e_" + str(aut.edge_number(e))), c])
                con1.append(impl)

                # part two -- both edges not in group w

                c1 = SATformula("&", [SATformula("w_" + src, negate=True), SATformula("w_"+dst, negate=True)])
                con2.append(SATformula("->", [SATformula("e_" + str(aut.edge_number(e))), c1]))
