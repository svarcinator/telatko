...
def filter_out_representants( aut, representants_set, scc_edg, scc_state_info):
    ...
    for i in range(len(scc_edg)):
        new_scc_edg.append(list(filter(lambda e: aut.edge_number(e) in representants_set, scc_edg[i])))
...
def scc_unoptimized_formula(
    ...
    for scc_edg_list in scc_edg:
        scc_edges_nums.append( list( map( lambda e: aut.edge_number( e ), scc_edg_list ) ) )
