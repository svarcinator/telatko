...
parameters $k,n$, it produces a \emph{quantified boolean formula
  (QBF)} which is satisfiable if and only if there exists an
equivalent automaton with the identical structure, $n$ acceptance
marks, and an acceptance formula in \todo{conjunctive?}  normal form
with $k$ clauses. The placement of the marks on transitions and the
...
equivalent automaton with the identical structure and $n$ acceptacne
marks, but not vice versa.
