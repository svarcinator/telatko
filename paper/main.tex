...
\todo{Sem nebo za tu dalsi subsection strcity analyzu slozitosti.}
...
\todo{Zminit, ze nase QBF formule navic rikaji, ze v zadne
  klauzulu/cude neni zaroven $\Fin m$ a $\Inf m$ a ze cela formule
  ma alepson 1 $\Inf$ nebo $\Fin$? Je to tak, Terko?}

\todo{Zminit, ze jsme zkouseli symmetry breaking, ale nefungoval?}
...
\begin{tabular}{lrlrrr}
  \toprule
  &&&& \multicolumn{2}{c}{automata with} \\ \cmidrule(lr){5-6}
  tool &\!\!\!version&homepage & T/O & 0--1 mark & \textbf{2+ marks}\\
  \midrule
  \texttt{ltl2dela} & 21.0 & \multirow{2}{*}{\url{https://owl.model.in.tum.de/}}
       & ? & ? & \textbf{129} \\
  \texttt{ltl2dgra} & 21.0 &
       & ? & ? & \textbf{234}\\
  \texttt{ltl2tgba} & 2.10.4 & \url{https://spot.lrde.epita.fr}
       & ? & ? & \textbf{70}\\
  \texttt{ltl3tela} & 2.2.0  & \url{https://github.com/jurajmajor/ltl3tela}
       & ? & ? & \textbf{91}\\
  \bottomrule
\end{tabular}
\end{table}
...
reducing of the number of cubes.
\todo{Vysvetlit, proc neredukujeme cubes?}
...
\todo{popisek Fig. 3 neni hezky. Je to wall time nebo CPU time?}
\begin{figure}[b!]
  \caption{Performance of \telatko on different automata sets. The $x$
    axis represents the number of automata that are processed by
    telatko in the time shown on the $y$ axis.}
\label{fig:quantile}
\bigskip
\centering
\input{quantile.tex}
