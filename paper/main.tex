...
\begin{lemma}
  A set $T\subseteq\delta$ is a cycle if and only if for each set of
  states $S\subseteq Q$ it holds that
  \begin{equation}
    \begin{minipage}{0.85\linewidth}
      \centering
      if $T$ contains a transition leading from a state in $S$ to a state outside $S$,\\
      then it also contains a transition leading from a state outside $S$ to a state in $S$.
    \end{minipage}
    % \begin{split}
    % \textrm{if $T$ contains a transition leading from a state in $S$ to a state outside $S$,}\\[-.5ex]
    % \textrm{then it also contains a transition leading from a state outside $S$ to a state in $S$.}
    % \end{split}
    \tag{$3$}\label{eq:lem3} 
  \end{equation}
\end{lemma}
\begin{proof}
  We first prove direction ``$\Longrightarrow$''. Let $T$ be a cycle
  and $S\subseteq Q$ be a set of states such that there exists a
  transition $t\in T$ leading from a state $p\in S$ to a state
  $q\not\in S$. The definition of a cycle says that there exists a
  path $t_1t_2\ldots t_n\in T^+$ from a state $p'$ back to $p'$ using
  all transitions of $T$. Hence, $t=t_i$ for some $1\le i\le n$. Then
  $t_{i+1}\ldots t_nt_1t_2\ldots t_{i-1}\in T^+$ is a path leading
  from the state $q\not\in S$ to the state $p\in S$. This path has to
  contain a transition $t'\in T$ leading from a state outside $S$ to a
  state in $S$.

  Now we prove the opposite direction ``$\Longleftarrow$''.
