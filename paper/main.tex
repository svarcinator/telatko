...
\item $\mcycle_2(\vec{e})$ is a balanced version. It says that every
  transition in $\Te$ has to have a preceding and a succeeding
  transition in $\Te$, which is a necessary condition on cycles, but
...
The lightweight version is based on the fact that every cycle is nonempty.
Indeed, the only restriction imposed by
\[
  \mcycle_1(\vec{e})=\bigvee_{t\in\delta}e_t
\]
is that $\Te$ is nonempty.
...
Formally, we define an equivalence on transitions such that for each
$t_1,t_2\in\delta$ we write $t_1\sim t_2$ whenever
$\mks(t_1)=\mks(t_2)$.
%This relation is clearly an equivalence.
We prove the following statement. 
\begin{lemma}
  Assume that there is a function $\rem$ and a formula $\psi$ such
  that for every nonempty set $T\subseteq\delta$ it holds that $T$
  satisfies $\varphi$ iff $\rem(T)$ satisfies $\psi$. Then we can
  safely assume that $\rem$ respects the quivalence $\sim$, that is
  $t_1\sim t_2\implies\rem(t_1)=\rem(t_2)$.
\end{lemma}
\begin{proof}
  \todo{Jan is here!}
\end{proof}

Formally, for each pair of transitions $t_1,t_2\in\delta$, we write
$t_1\sim t_2$ whenever $\mks(t_1)=\mks(t_2)$. This relation is clearly an equivalence.
...
We show that if this formula holds for some $\rem$ and $\psi$,
then it also holds for some $\rem'$ and $\psi$ such that $\rem'$
assigns the same sets of acceptance marks to transitions originally
marked by the same sets of marks.
