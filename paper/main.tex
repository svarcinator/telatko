...
% $\pi$.
Further, each set $\minf(\pi)$ for a run $\pi$ is a cycle and
vice versa. Hence, our reduction methods is looking for a new
acceptance formula $\psi$ and function $\rem$ such that for each cycle
$T\subseteq\delta$, it holds that $T$ satisfies $\varphi$ if and
only if $\rem(T))$ satisifes $\psi$. This can be formally written
by the formula
\[
  \forall T\subseteq\delta~.~\mcycle(T)\implies(T~\msat~\varphi ~\iff~ \rem(T)~\msat~\psi).
\]
