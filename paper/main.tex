...
parameters $k,n$, it produces a QBF formula which is satisfiable if
and only if there exists an automaton with the same structure, $n$
acceptance marks, an acceptance formula in disjunctive normal form
with $k$ clauses, and the same set of accepting runs as the original
...
existence of an automaton with the same structure, $n$ acceptance
...
A transition $t=(p,a,M',q)\in\delta$ is the transition leading from
state $p$ to state $q$ labelled with $a$ and acceptance marks $M'$. We
use $M(t)$ to refer to its set of acceptance marks $M'$. We extend
this notation to a set $T\subseteq\delta$ of transitions such that
$M(T)=\bigcup_{t\in T}M(t)$ denotes the set of acceptance marks that
appear on transitions in $T$.

\todo{Rozmyslet si, jak dal.}
...
infinitely many times in $\pi$. Further, let $M(\pi)$ denote the set
of acceptance marks that occur in infinitely many transitions of run
$\pi$, i.e., $M(\pi)=M(\minf(\pi))$. Run $\pi$ is \emph{accepting} iff
$M(\pi)$ satisfies the formula $\varphi$, where $M(\pi)$ satisfies
$\Inf m$ iff $m\in M(\pi)$ and $M(\pi)$ satisfies $\Fin m$ iff
$m\not\in M(\pi)$. The \emph{language} of automaton $\mA$ is the set
...
the set $\inf(\pi)$ of transitions appearing infinitely often in
$\pi$.
% \[
%   M(\pi)=\bigcup_{(q_i,u_i,M_i,q_{i+1})\in\minf(\pi)}M_i
% \]
Further, each set of the form $\inf(\pi)$ is a cycle and vice versa.
Hence, our reduction methods is looking for 



following simple observation. Definovat $inf(\varphi)$?
