...
(linear temporal logic) formulas with tools \texttt{ltl2dela}
(known as Delag)~\cite{muller.17.gandalf} or \texttt{ltl2dgra}
(known as Rabinizer~4)~\cite{kretinsky.18.cav} of the Owl library,
...
tool \telatko implementing the reduction technique.
...
By choosing an appropriate format of the acceptance condition, one can
easily represent many classical kinds of automata over infinite words,
e.g. a B\"uchi automaton is a TELA with $M = \{\tacc{1}{1}\}$,
$\varphi = \Inf\tacc{1}{1}$, and the acceptance mark \tacc{1}{1} being
placed on all accepting transitions.
Similarly, a Rabin automaton with $k$ acceptance
pairs has $M = \{\tacc{5}{1}, \tacc{3}{1'} \ldots, \tacc{5}{k}, \tacc{3}{k'}\}$
and $\varphi = (\Fin\tacc{5}{1} \land \Inf\tacc{3}{1'}) \lor \ldots
\lor (\Fin\tacc{5}{k} \land \Inf\tacc{3}{k'})$.
A state-based acceptance can be represented by TELA by placing the
corresponding acceptance marks on all transitions leaving the accepting
states.
...
easily adapted to look for $\psi$ in conjunctive normal form (CNF)
...
follows. Three versions of the subformula $\mcycle(\vec{e})$ will
be presented.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[smallautomaton]
    \begin{scope}[initial angle=90,xshift=-4cm]
    ...
    \begin{scope}[initial angle=90,densely dotted,black!35,xshift=4cm]
      \node[state,initial,solid,draw=black] (p) at (0,0) {};
      \node[state,solid,draw=black] (q) at (1.8,0) {};
      \path[->,auto]
      (p) edge[out=-150,in=150,looseness=10,black,solid] (p)
...
\begin{lemma}
  ...
  states $S\subseteq Q$ one of the following conditions hold.
  \begin{enumerate}[A.]
...
$\true$, then the name of the output automaton, included in the
generated HOA, encodes the reason for
...
literature, using all available formula sets implemented in
\texttt{genltl} 2.10.4.
The parametrized formulas were generated for all
...
limit. The reduction ratio drop in higher levels, e.g. between level~1
and level~2 for \texttt{ltl2dgra}, happens due to the QBF solver timeouts
occuring earlier as the formula being solved in higher level is more
complex. For \texttt{ltl2dela} and \texttt{ltl2dgra}, the automata sets
do not contain any automaton where level~2 or level~3 achieves a better
result than level~1. However, all levels contribute to the reductions in
