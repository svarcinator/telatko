...
The connection formula distinguishes the remaining three levels of simplification. Previously we denoted it as $\varphi_{C}$.
...
In other words, formula $\varphi_{C}$ holds if the \emph{selection of edges} has the required property. If not, the formula $\varphi_{C}$ is $\false$, and then the whole formula $\varphi$ is trivially satisfied. Formula $\varphi_{C}$ consists of two parts.
...
The second part of $\varphi_{C}$ specifies the properties of \emph{relevant set} that the edges that belong together to \emph{selected edges} need to have. We divided these properties into two levels Level 2, Level 3, Level 4, all described in ensuing sections.
...
The simplification Level 3 ensures that \emph{selection of edges} is connected, but we can go one step further and specify that \emph{selection of edges} is in fact a \emph{relevant set}.

The intuition by simplification Level 4 is that the states of an automaton are again divided into two groups -- the states that belong to \emph{selected states} and the states do not belong to \emph{selected states}.  We consider three possible scenarios that can occur while the states are divided into these two groups:
\begin{enumerate}
