...
Before we give a definition of $\mcycle_3(\vec{e})$, we prove that
cycles can be characterised in the following way.
\begin{lemma}
  A set $T\subseteq\delta$ is not a cycle if and only if there exists a set of
  states $S\subseteq Q$ such that 
  \begin{enumerate}
  \item some transition $t\in T$ leads from a state in $S$ to a state
    outside $S$ and
  \item there is no transition in $T$ leading from a state outside $S$
    to a state in $S$.
  \end{enumerate}
  \todo{Nebo neprazdna cast $T$ vede z $S$ do $S$ a jina neprazdna
    cast $T$ vede z doplňu $S$ do doplňku $S$ a žádná hrana to
    nekrosí.}
\end{lemma}
...
\begin{figure}[t]
  \centering
  \begin{tikzpicture}[smallautomaton]
    \begin{scope}[initial angle=90]
      \node[state,initial] (p) at (0,0) {};
      \node[state] (q) at (2,0) {};
      \path[->,auto]
      (p) edge[out=-150,in=150,looseness=10] pic {acc={0}{0}} pic {l=$1$} (p)
      (p) edge[out=30,in=150] pic {acc={1}{1}} pic {l=$2$} (q)
      (q) edge[out=30,in=-30,looseness=10] pic[pos=.33] {acc={2}{2}} pic[pos=.67] {acc={3}{3}} pic {l=$3$} (q)
      (q) edge[out=-150,in=-30] pic {acc={3}{3}} pic {l=$4$} (p);

      \node[acclabel] at (1,-1.5)
      {$(\Fin\tacc{1}{1}\wedge\Inf\tacc{2}{2})\vee(\Inf\tacc{0}{0}\wedge\Fin\tacc{1}{1}\wedge\Inf\tacc{3}{3})$};
    \end{scope}

% \begin{scope}[initial angle=90]
% \node[state,initial] (phi) at (0,0) {$\varphi\vphantom{\G}$};
% \node[state] (Fb) at (1.5,-1.5) {$\F b$};
% \path[->,auto]
% (phi) edge[loop left]
%   pic {acc={3}{1}} pic {l=$\true$} (phi)
% (phi) edge[loop right]
%   pic {acc={1}{3}} pic {l=$a$} (phi)
% (phi) edge[out=-135,in=-105,looseness=10]
%   pic[pos=.12] {acc={2}{2}} pic[pos=.3,left] {l=$b$} (phi)
% %(phi) edge[out=-45,in=45] (Fb)
% (phi) edge[out=-45,in=105]
%   node[above right,inner sep=.3,pos=.45] {$\bar{b}$} (Fb)
% (phi) edge[out=-45,in=-75,looseness=10]
%   pic[pos=.12] {acc={2}{2}} (phi)
% (Fb) edge[loop right]
%   pic {acc={3}{1}} pic {l=$\bar{b}$} (phi)
% (Fb) edge node {$b$} + (0,-1.1)
% ;
% \node[acclabel] at (.75,-3.2)
%   {$\Fin\tacc{3}{1}\wedge(\Fin\tacc{2}{2}\vee\Fin\tacc{1}{3})$};
% \end{scope}

% \begin{scope}[xshift=4.8cm,initial angle=90]
% \node[state,initial] (phi) at (0,-.7) {$\varphi\vphantom{\G}$};
% \path[->,auto]
% (phi) edge[loop left] 
%   pic {acc={3}{1}} pic {l=$\true$} (phi)
% (phi) edge[loop right]
%   pic {acc={1}{3}} pic {l=$a$} (phi)
% (phi) edge[out=-135,in=-105,looseness=10]
%   pic[pos=.75] {accsq={0}{5}} pic[pos=.15] {acc={2}{2}}
%   pic[left] {l=$b$} (phi)
% (phi) edge[out=-45,in=-75,looseness=10]
%   pic[pos=.15] {acc={2}{2}} pic[pos=.75] {acc={4}{4}}
%   pic[right] {l=$\bar{b}$} (phi)
% ;
% \node[acclabel,align=center] at (0,-2.8)
%   {$\Fin\tacc{3}{1}\wedge{}$\\$(\Fin\tacc{2}{2}\vee\Fin\tacc{1}{3})
%    \wedge{}$\\$(\Fin\tacc{4}{4}\vee\Inf\taccsq{0}{5})$};
% \end{scope}

  \end{tikzpicture}
  \caption{An automaton showing different reduction result for various reduction levels. \todo{Overit a dodelat!}}
  \label{fig:levels}
\end{figure}
