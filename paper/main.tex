...
It remains to define the subformula $\mcycle(\vec{e})$ with the intended meaning
\[
  \vec{e}\textrm{ represents a cycle} \iff \mcycle(\vec{e}).
\]
In fact, only the direction ``$\Longrightarrow$'' is required for
correctness of our reduction method. If $\mcycle(\vec{e})$ holds for
some valuation of $\vec{e}$ that does not represent a cycle, then the
...
\item $\mcycle_1(\vec{e})$ is a lightweight version, which puts
  only minimal requirements on $\vec{e}$. It does not use the
  information about automaton structure, but it comes with an
  interesting simplification of the whole formula $\Phi_{C,K}$.
\item $\mcycle_2(\vec{e})$ is a balanced version. It uses the
  information about the automaton structure to identify many
  valuations of $\vec{e}$ that are not cycles. It is relatively
  simple, but it is still satisfied by some valuations of $\vec{e}$
  not corresponding to cycles.
\item $\mcycle_3(\vec{e})$ is a strict version satisfied only by
  the valuations of $\vec{e}$ representing cycles. Unfortunately,
  it uses additional quantified variables. 
\end{itemize}
...
\]
is that $\vec{e}$ represents a nonempty set.

The fact that this version do 
