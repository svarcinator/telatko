...
requirement clearly guarantees the equiavalence of $\mA$ and $\mB$. We
note that preserving the acceptance of individual runs is not a
necessary condition for language equivalence of $\mA$ and $\mB$ and
there exist automata where relaxing this requirement can lead to
a bigger reduction of acceptance marks (see
Figure~\ref{fig:counter}). However, looking for $\psi$ and $\rem$
preserving the acceptance of individual runs makes the problem easier
as we can, for example, ignore the labelling of transitions by the
elements of $\Sigma$.
\begin{figure}[t]
  \centering
  ...
  \caption{Both automata accept the same language of words
    $w\in\{a,b\}^\omega$ that contain infinitely many occurences of
    both $a$ and $b$. Each accepting run of the left automaton has to
    contain infinitely many occurrences of both transitions looping on
    the initial state. Hence, there does not exist any automaton with
    the same accepting runs as the left automaton and less than two
    accetpance marks.}
  \label{fig:counter}
\end{figure}

\fixme{TADY TO HONZA JESTE UPRAVUJE}
...
\begin{lemma}
  ...
  there is a run $\pi$ such that $T=\minf(\pi)$.
\end{lemma}



As $\minf(\pi)$ is the set of transitions that appear infinitely many
times in $\pi$, there has to be a suffix $\pi'$ of $\pi$ containing
only transitions of $\minf(\pi)$. Let $p$ be the first state of
$\pi'$. As each transition of $\pi'$ appears infinitely many times in
$\pi$ and thus also in $\pi'$, there has to be a finite prefix $\rho$
of $\pi'$ such that $\rho$ is a path from $p$ to $p$ that contains all
transitions of $\minf(\pi)$. In other words, the set $\minf(\pi)$ of a
run $\pi$ is always a cycle. Moreover, as we assume that 
%\footnote{We can also prove that for each cycle $T$ there exists a run $\pi$ such that $T=\minf(\pi)$.}


\fixme{ODTUD DAL JE TO PUVODNI, ale taky se to trochu bude menit!}
...
To evaluate our reduction technique, we considered all sets of LTL
formulas from literature as they are provided by the tool
\texttt{genltl} of the Spot library~\cite{duret.16.atva2}~2.10.4, where
parametrized formulas were generated for all combinations of
parameter values from 1 to 4. We used the tool \texttt{ltlfilt} of the
Spot library to simplify the formulas and remove
duplicates. Altogether, we have generated 348 unique LTL
...
Rabinizer~4)~\cite{kretinsky.18.cav}, and to nondeterministic TELA by
another two state-of-the-art translators, namely \texttt{ltl2tgba}
from the Spot library~\cite{duret.16.atva2} and
\texttt{ltl3tela}~\cite{major.19.atva}. Three of the considered
...
incremental approach and without reducing the number of cubes. We
...
Table~\ref{tab:cumm-data} shows the cumulative numbers of marks in the
input automata sets and after each reduction, together with the
...
limit. The timeout of the last QBF query means that the automaton may
be potentially further reduced if a longer time limit is used. One can
observe that a higher level sometimes achieves a smaller reduction
than a lower level (e.g., compare level~1 and level~2 for
\texttt{ltl2dgra}). This is caused by the QBF solver timeouts occuring
earlier as formulas constructed by the higher level are more
complex. For \texttt{ltl2dela} and \texttt{ltl2dgra}, the automata
sets do not contain any automaton where level~2 or level~3 achieves a
better result than level~1. However, all levels contribute to the
reductions in the multi-level setting.
...
\begin{table}[t]
  \caption{The cumulative numbers of acceptance marks before and after
    ...
    instances where the last call to the QBF solver timed out.  }
\label{tab:cumm-data}
\centering
