...
expect that ACD could potentially be used to reduce the number of
...
\[
   ...
   i_{c,k}\vee\neg f_{c,k}).
\]
The added part says that any cube does not contain both $\Inf k$ and
$\Fin k$ for any $k$. A cube with both $\Inf k$ and $\Fin k$ would be
useless as it cannot be satisfied by any run.
