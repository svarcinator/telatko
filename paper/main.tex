...
\begin{abstract}
  ...
  % acceptance marks.
  In fact, we present three formula constructions trading speed for
  higher reduction efficiency. We evaluated our reduction technique on
  TELA produced by state-of-the-art tools of the libraries Owl and
  Spot and by the tool \texttt{ltl3tela}. The technique reduced some
  acceptance marks in automata produced by all the tools. The achieved
  reduction on automata produced by tools of the Owl library was
  between 30 and 40\%.
\end{abstract}
...
\texttt{ltl2dela} (known as Delag)~\cite{muller.17.gandalf}),
\texttt{ltl3tela}~\cite{major.19.atva},
\texttt{ltl2dgra} (known as Rabinizer~4)~\cite{kretinsky.18.cav},
or \texttt{ltl2tgba} which is a part of the Spot library~\cite{duret.16.atva2}.
There are also some algorithms processing
...
modest for TELA produced by \texttt{ltl3tela} and Spot,
it is substantial for automata produced by tools of the Owl library.
...
The reduction procedure is given in Algorithm~\ref{alg:single}. Besides
the reduction of acceptance marks, the algorithm also reduces the
number of clauses in acceptance formula if the last argument
$\mathit{reduceC}$ is set to $\true$. In the first step, the procedure
checks whether the input automaton represents the empty language and
if this is the case, it returns the same automaton with acceptance
conditon $\false$ and with all acceptance marks removed. Otherwise it
gradually decreases the number of marks until 0 is reached or the QBF
solver behind the function $\mathit{satisfiable}(\Phi)$ fails to
reduce the number of marks, i.e., it claims unsatisfiability of $\Phi$
or it runs out of resources. In the next step, if $\mathit{reduceC}$
is set to $\true$ then the procedure gradually reduces the number of
clauses. It stops if no equivalent automaton with less clauses is
found or the number of clauses is reduced to one. Note that the
acceptance condition with 0 clauses is $\false$ and this case is
treated at the beginning of the procedure. Finally, if the procedure
succeeds to reduce the number of marks or clauses, if constructs the
modified automaton. Otherwise, it returns the original automaton.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[smallautomaton]
    ...
    \end{scope}
    \begin{scope}[initial angle=90,xshift=-3.5cm,yshift=-4cm]
...
combining reductions of all levels. We call this aproach
...
\begin{algorithm}[t]
  ...
  \PI{$\mathit{MultiLevelReduction}(\mA,\mathit{reduceC})$}{
    ...
    \KwOut{an equivalent TELA with the same structure as $\mA$ and with at most as many acceptance marks as in $\mA$} %with the same or lower number of acceptance~marks}
    \;
    $\mA\gets\mathit{SingleLevelReduction}(\mA,1,\mathit{reduceC})$\;
    $\mA\gets\mathit{SingleLevelReduction}(\mA,2,\mathit{reduceC})$\;
...
combinations of parameter values from 1 to 4. Altogether, we have
generated 348 unique LTL formulas. These have been translated to
deterministic TELA by two state-of-the-art translators from the Owl
...
produced by individual tools contain less than 348 automata as not all
of the formulas are successfully translated due to timeout of 60
seconds. Further, we have removed automata with 0 or 1 acceptance
marks as there is little point reducing these. Table
\ref{tab:ltltranslators} shows the exact versions of used translators,
the homepage of the used tools and the number of automata produced by
the given tool from the LTL formulas after filtering.

We have used multiple other helper tools in the experimental
evaluation.  Alongside \texttt{genltl}, we have used \texttt{ltlfilt}
to simplify the formulas and to remove duplicates, and
\texttt{autcross} to compare automata and measure the running times.

Spot version 2.10.4 and Z3 version 4.8.13 have been used in the
experimental evaluation.

For each automata set (that is, the output of an individual LTL
translator), we have applied all single-level reductions and the
...
set to 30 seconds.
%We have again filtered out the automata for which
%any of the four \telatko runs timed out after 120 seconds. We do this
%to make sure the numbers of acceptance marks can be meaningfully
%compared between different reduction levels of the same automata set.
Additionally, we have filtered one automaton from the \texttt{ltl3tela} set
that cannot be processed by \telatko due to an extensive use of system
resources.
...
columns \textit{solver response} and \textit{solver timeout} show the
number of automata for which the last query to QBF solver has been
successfully answered, or the solver timed out within a 30 seconds
limit to a single query, respectively. The timeout of the last QBF
query implies that the automaton could be potentially further reduced
by the reduction with a longer time limit.
...
\texttt{ltl3tela} & 2.2.0  & \url{https://github.com/jurajmajor/ltl3tela} & 90\\
\bottomrule
\end{tabular}
\end{table}
...
The columns \textit{solver response} and
...
example, in 22 cases, we have successfully reduced 7 or more
...
under a second and the high cummulative running times are caused by
few complicated automata.
...
with less acceptance marks and without changes to automata structure.
...
good trade-off between speed and efficiency by choosing precision of
...
Do submission:
\begin{itemize}
\item upravit nazev a abstrakt, zduraznit uzitecnost TELA pro concurrency (analysis of probabilistic systems...)
\item do experimentu napsat, jaky hardware pouzivame (merit cas bez toho nedava smysl)
\item spustit experimenty bez timeoutu na telatko (?a delsi timeout na QBF?)
\item prelestit text v poslednich dvou kapitolach
\end{itemize}
