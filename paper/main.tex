...
TELA have attracted a lot of attention during the last %five
...
example, a transition-based B\"uchi automaton is a TELA with
$M = \{\tacc{1}{1}\}$, $\varphi = \Inf\tacc{1}{1}$, and the acceptance
mark \tacc{1}{1} being placed on all accepting transitions.
Similarly, a transition-based Rabin automaton with $k$ acceptance
pairs has
$M = \{\tacc{5}{1}, \tacc{3}{1'}, \ldots, \tacc{5}{k}, \tacc{3}{k'}\}$
and
$\varphi = (\Fin\tacc{5}{1} \land \Inf\tacc{3}{1'}) \lor \ldots \lor
(\Fin\tacc{5}{k} \land \Inf\tacc{3}{k'})$.  \fixme{A state-based acceptance
can be represented by TELA by placing the corresponding acceptance
marks on all transitions leaving the accepting states.} 
...
follows. \fixme{The subformula $\mcycle(\vec{e})$ will be defined as the last
...
\begin{figure}[t]
  ...
  \caption{An automaton structure where the transition set forms a cycle
    (left) and two sets $\Te$ that
    are not cycles even if $\mcycle_2(\vec{e})$ holds (middle and
    right). The transition labels and acceptance marks are not depicted.}
  \label{fig:inter}
\end{figure}
...
satisfiability of QBF. Our tool is available at
\begin{center}
...
\fixme{We have used the tool \texttt{genltl} of the Spot
library~\cite{duret.16.atva2} to generate LTL formulas from
literature, using all available formula sets implemented in
\texttt{genltl} 2.10.4.} The parametrized formulas were generated for all
...
limit. \fixme{The reduction ratio drop in higher levels, e.g.~between level~1
...
numbers, the latter represents number of automata where the attempt to reduce
