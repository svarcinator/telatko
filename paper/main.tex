...
set $L(\mA)=\{u\in \Sigma^{\omega}\mid\textrm{there is an accepting run of }\mA\textrm{ over }u\}$.
% such that $\mA$ has
...
For each mark $m\in M$, let $\delta_m$ denotes the set of all
transitions marked with $m$, i.e.,
$\delta_m=\{(p,a,M',q)\in\delta\mid m\in M'\}$. Further, a \emph{path}
from a state $p$ to a state $q$ is a finite sequence of adjacent
...
denotes the set of all transitions between states in $S$.
...
$Q_1,Q_2,\ldots,Q_n \in \{\forall, \exists\}$ and $\psi$ is a Boolean
formula.
