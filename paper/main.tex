...
Now we present a formula optimization based on the fact that every
cycle is completely included in the transition set $\delta_S$ of some
SCC $S$. Note that we cannot reduce the number of acceptance marks in
each SCC separately as all SCCs have to share one acceptance formula.
However, for each SCC we can build a $\Phi_{C,K}$ formula separately and
then we can put all these formulas into one conjunction such that they
share the variables $\vec{i},\vec{f}$.

\todo{Honza je tu!} 

For a given automaton $\mA=(Q,M,\Sigma,\delta,q_I,\varphi)$, we first
compute all SCCs. Each SCC $S$ induces an automaton $(S,M,\Sigma,q',\delta_S,\varphi)$, where $q'$ is an arbitrary state
of $S$. 


the optimization saying that So far, we have not use the fact that each cycle is A 
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[smallautomaton]
    \begin{scope}[initial angle=90,xshift=-3.5cm]
      ...
      {\small$(\Fin\tacc{1}{1}\wedge\Inf\tacc{2}{2})\vee(\Inf\tacc{0}{0}\wedge\Fin\tacc{1}{1}\wedge\Inf\tacc{3}{3})$};
    \end{scope}
    \begin{scope}[initial angle=90,xshift=3.5cm]
      \node (l) at (1,1.3) {level 1:};
      \node[state,initial] (p) at (0,0) {};
      \node[state] (q) at (2,0) {};
      \path[->,auto]
      (p) edge[out=-150,in=150,looseness=10] pic {acc={0}{0}} pic {l=$a$} (p)
      ...
      \node[acclabel] at (1,-1.4)
      {\small$(\Fin\tacc{0}{0}\wedge\Fin\tacc{1}{1}\wedge\Fin\tacc{2}{2})\vee(\Inf\tacc{0}{0}\wedge\Fin\tacc{1}{1}\wedge\Inf\tacc{2}{2})$};
    \end{scope}
    \begin{scope}[initial angle=90,xshift=-3.5cm,yshift=-4cm]
      \node (l) at (1,1.3) {level 2:};
      \node[state,initial] (p) at (0,0) {};
      \node[state] (q) at (2,0) {};
      \path[->,auto]
      ...
      \node[acclabel] at (1,-1.4)
      {\small$(\Fin\tacc{0}{0}\wedge\Inf\tacc{1}{1})\vee(\Inf\tacc{0}{0}\wedge\Fin\tacc{1}{1})$};
    \end{scope}
    \begin{scope}[initial angle=90,xshift=3.5cm,yshift=-4cm]
      \node (l) at (1,1.3) {level 3:};
      \node[state,initial] (p) at (0,0) {};
      \node[state] (q) at (2,0) {};
      \path[->,auto]
      (p) edge[out=-150,in=150,looseness=10] pic {acc={0}{0}} pic {l=$a$} (p)
...
used tools and the number of input automata produced by the given tool from
the LTL formulas.
...
\begin{tabular}{lr@{~~~}lr}
\toprule
tool        & version&homepage & \# formulas \\
\midrule
\texttt{ltl2tgba} & 2.10.4 & \url{https://spot.lrde.epita.fr} & 404\\
\texttt{ltl3tela} & 2.2.0  & \url{https://github.com/jurajmajor/ltl3tela} \todo{RELEASE} & 376\\
Delag & 21.0 & \multirow{2}{*}{\url{https://owl.model.in.tum.de/}} & 397\\
Rabinizer & \todo ? & & 386\\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
& & marks & \% red. & time & timeouts \\
\midrule
\multirow{4}{*}{\rotatebox[origin=c]{90}{\makecell{Delag \\ (420 marks)}}} & level 1 & 240 & 12.3 & 133.7 & 42 \\
& level 2 & 240 & 12.3 & 133.7 & 42 \\
& level 3 & 240 & 12.3 & 133.7 & 42 \\
& multilevel & 240 & 12.3 & 133.7 & 42 \\
\midrule
\multirow{4}{*}{\rotatebox[origin=c]{90}{\makecell{Rabinizer 4 \\ (420 marks)}}} & level 1 & 240 & 12.3 & 133.7 & 42 \\
& level 2 & 240 & 12.3 & 133.7 & 42 \\
& level 3 & 240 & 12.3 & 133.7 & 42 \\
& multilevel & 240 & 12.3 & 133.7 & 42 \\
\bottomrule
