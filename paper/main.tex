...
\authorrunning{J.~Major, T.~Schwarzová and J. Strej\v{c}ek}
 \author{Juraj Major \and Tereza Schwarzová \and Jan Strej\v{c}ek}
 \institute{%
   Masaryk University, Brno, Czech Republic\\ \email{\{major,xschwar3,strejcek\}@fi.muni.cz}
 }
...
\begin{abstract}
  ...
  its accepting formula is a positive Boolean combination of terms
  ...
  marks. We introduce a new technique reducing the number of
  acceptance marks in a TELA with use of QBF solving. The technique
  produces QBF formulas 

  \textbf{Honza je tu}

  without altering its structure and the
  represented language. The technique constructs a \emph{quantified
    Boolean formula (QBF)}


  whose satisfiebility impli a QBF formula parametrized
  which is satisfiable only if  


  solver for an
  acceptance condition with fewer acceptance marks and the placement
  of these marks in the automaton structure. Both techniques are
  implemented and experiments show that the number of acceptance marks
  in TELA produced by state-of-the-art tools Delag, ltl3tela, Rabinizer~4,
  and SPOT can be sometimes reduced. In the case of Rabinizer~4, we
  reduced the cummulative number of acceptance marks to less than one
  half.
\end{abstract}
...
%A transition $(p,a,M',q)\in\delta$ is a transition leading from state $p$ to state $q$ labelled with $a$ and acceptance marks $M'$
...
% Finaly, \emph{loop} is a path from a state $p$ to the same state.
