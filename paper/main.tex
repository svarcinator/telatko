...
\item For each $c\in\{1,2,\ldots,C\}$ and acceptance mark $k\in N_K$,
  ...
  \[i_{c,k}=\left\{
      \begin{array}{ll}
        1 & \textrm{if $c^\textrm{th}$ clause of $\psi$ contains $\Inf k$}\\
        0 & \textrm{otherwise}    
      \end{array}\right.
  \]
  \[f_{c,k}=\left\{
      \begin{array}{ll}
        1 & \textrm{if $c^\textrm{th}$ clause of $\psi$ contains $\Fin k$}\\
        0 & \textrm{otherwise}    
      \end{array}\right.
  \]
...
\]
In fact, our reduction methods remains correct whenever ``implies'' holds.
If $\mcycle(\vec{e})$


  $\vec{e}$ represents a cycle. In fact, $\mcycle(\vec{e})$ needs to be
satisfied by all cycles, but our reduction technique remains correct
even if it is satisfied also by some sets $T\subseteq\delta$ that are
not cycles. The only consequence is that



Note that our reduction method remains
correct even if $\mcycle(\vec{e})$ isAs we will see, However, our reduction method remains
correct.

The intended meaning
of this formula is We offer three definitions:

\todo{Jan is here!}

\subsection{Lightweight version of $\mcycle(\vec{e})$ (level 1)}


\subsection{Balanced version of $\mcycle(\vec{e})$ (level 2)}


\subsection{Strict version of  $\mcycle(\vec{e})$ (level 3)}
