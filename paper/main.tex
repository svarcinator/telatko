...
\begin{abstract}
  ...
  number of %different
  acceptance marks in TELA based on satisfiability of \emph{quantified Boolean
    formulas (QBF)}.
...
cubes and formula $\true$ corresponds to one cube with zero terms.
...
that our reduction method can be easily adapted to look for $\psi$ in
...
\item For each $c\in\{1,2,\ldots,C\}$ and acceptance mark $k\in N_K$,
  ...
  cube of $\psi$ contains terms $\Inf k$ or $\Fin k$, respectively.
  \[i_{c,k}=\left\{
      \begin{array}{ll}
...
one cube. Recall that the presence of terms $\Inf k$ and $\Fin k$ in
...
reducing of the number of cubes. The timeout for each QBF query was
set to 30 seconds.
%
