...
automaton will use the acceptance marks $N_K=\{1,2,\ldots,K\}$. First
we desribe Boolean variables used in the constructed QBF formulas.
\begin{itemize}
\item The fact whether a transition $t$ is in the set $T$ is
  represented by a boolean variable $e_t$. By $\vec{e}$ we denote the vector
  of length $|\delta|$ of all variables $e_t$.
\item 
\end{itemize}
...
\[
  \forall \vec{e}~.~\mcycle(\vec{e})\implies\big(\msat_\varphi(\vec{e}) ~\iff~ \msat(\vec{e},\vec{k},\vec{p})\big).
\]
