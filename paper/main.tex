...
\author{Tereza Schwarzov\'{a} \and
        Jan Strej\v{c}ek\orcidID{0000-0001-5873-403X}\thanks{Corresponding author: \email{strejcek@fi.muni.cz}} \and
        Juraj Major}
\authorrunning{T. Schwarzov\'{a}, J. Strej\v{c}ek, and J. Major}
\titlerunning{Reducing Acceptance Marks in TELA by QBF-solving}
...
says that for every nonempty set $T\subseteq\delta_S$ whenere $S$ is
an SCC, $T$ satisfies $\varphi$ if and only if $\rem(T)$ satisfies
$\psi$. Note that the formula is not affected by the placement of
transitions in the automaton. \todo{Tady to predelat, aby to
  reflektovalo $T\subseteq\delta_S$.}  The only aspect of a transition
$t$ reflected by the formula is its set of marks $\mks(t)$. Hence, we
do not have to distinguish between transitions with the same sets of
marks.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[smallautomaton]
    ...
    \begin{scope}[initial angle=90,densely dotted,black!20,xshift=4cm]
      \node[state,initial,solid,draw=black] (p) at (0,0) {};
      \node[state,solid,draw=black] (q) at (1.8,0) {};
      \path[->,auto]
      (p) edge[out=-150,in=150,looseness=10,black,solid] (p)
...
\begin{proof}
  ...
  $\pre$ of states from which $p$ is reachable via transtitions of $T$.
  \[
    \begin{array}{r}
...
The second optimization is specific for $\Phi_{3,C,K}$. In
...
semantics of $\mcycle_3(\vec{e})$, but our prelimnary experiments
...
\begin{algorithm}[t]
  ...
  %       \KwRet $(Q,\emptyset,\Sigma,\delta',q_I,\false)$ where $\delta'$ is $\delta$ with all acceptance marks removed
  %     }
    $C_\mA\gets$ the number of clauses in the formula $\varphi$ transformed to~DNF\;
    $K_\mA\gets$ the number of acceptance marks in $\mA$\;
    $C\gets C_\mA$\;
    $K\gets K_\mA$\;
...
did not decided within the time limit. If $X$ is \texttt{T}, a longer
